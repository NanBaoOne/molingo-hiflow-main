...
def arg_parse(is_train=False):
    ...
    parser.add_argument('--second_stage', type=str, default='molingo', choices=['molingo', 'hiflow'],
                        help='second-stage latent generator')
    parser.add_argument('--time_patch', type=int, default=2, help='HiFlow-SAE temporal patch size')
    parser.add_argument('--hiflow_scales', type=str, default='0.3,0.6,1.0',
                        help='comma-separated pyramid scales for HiFlow-SAE')
    ...
    parser.add_argument('--sae_name', type=str, default=None, help='alias of --vae')
    parser.add_argument('--sae_ckpt_name', type=str, default='net_best_fid.ckpt')
    ...
    if opt.sae_name is not None:
        opt.vae = opt.sae_name
    else:
        opt.sae_name = opt.vae

    args = vars(opt)
    opt.name = f'{opt.second_stage}_{opt.exp_name}_{opt.model_size}_vae_{opt.vae}'
