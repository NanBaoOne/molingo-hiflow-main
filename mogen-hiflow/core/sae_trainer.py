        ...
        while epoch < self.opt.max_epoch:
            ...
            for i, batch_data in enumerate(train_loader):
                ...
                if global_step % 100 == 0:
                    progress_bar.set_postfix(loss=float(loss.item()), lr=self.lr_scheduler.get_last_lr()[0])

                if global_step % animation_steps == 0:
                    self.visualize(global_step, batch_data)

                if global_step % validation_steps == 0:
                    cur_mpjpe, cur_fid = eval_vae_ms(
                        test_loader,
                        self.vae_model,
                        self.ms_wrapper,
                        self.datamodule,
                    ...
                    is_best_mpjpe = cur_mpjpe < min_mpjpe
                    is_best_fid = cur_fid < min_fid

                    metrics_path = os.path.join(self.opt.model_dir, "eval_metrics.txt")
                    write_header = not os.path.exists(metrics_path)
                    with open(metrics_path, "a") as f:
                        if write_header:
                            f.write(
                                "global_step,epoch,mpjpe,fid,"
                                "is_best_mpjpe,is_best_fid,"
                                "best_mpjpe_before,best_fid_before,lr\n"
                            )
                        f.write(
                            f"{global_step},{epoch},{cur_mpjpe:.6f},{cur_fid:.6f},"
                            f"{int(is_best_mpjpe)},{int(is_best_fid)},"
                            f"{min_mpjpe:.6f},{min_fid:.6f},"
                            f"{self.lr_scheduler.get_last_lr()[0]:.8e}\n"
                        )
                    ...
                    if is_best_mpjpe:
                        min_mpjpe = cur_mpjpe
                        save_path = os.path.join(self.opt.model_dir, "net_best_mpjpe.ckpt")
                        ckpt = dict(
                        ...
                        print(
                            f"Saved best MPJPE model to {save_path}: "
                            f"mpjpe={cur_mpjpe:.3f}, fid={cur_fid:.3f}, step={global_step}"
                        )

                    if is_best_fid:
                        min_fid = cur_fid
                        save_path = os.path.join(self.opt.model_dir, "net_best_fid.ckpt")
                        ckpt = dict(
                            state_dict=self.vae_model.state_dict(),
                            global_step=global_step,
                            epoch=epoch,
                        ...
                        torch.save(ckpt, save_path)
                        print(
                            f"Saved best FID model to {save_path}: "
                            f"fid={cur_fid:.3f}, mpjpe={cur_mpjpe:.3f}, step={global_step}"
                        )
