...
def eval_molingo_during_training(out_dir, val_loader, model_without_ddp, vae_model, datamodule, ema_params, ep, cfg,
    ...
    if rank == 0:
        metrics_path = pjoin(out_model_dir, "eval_metrics.txt")
        write_header = not os.path.exists(metrics_path)
        with open(metrics_path, "a") as f:
            if write_header:
                f.write("epoch,fid,top1,top2,top3,matching_score,matching_score_real,real_top1,real_top2,real_top3\n")
            f.write(
                f"{ep},{fid_tmr:.6f},{RP_tmr_pred[0]:.6f},{RP_tmr_pred[1]:.6f},{RP_tmr_pred[2]:.6f},"
                f"{matching_score_tmr_pred:.6f},{matching_score_tmr_real:.6f},"
                f"{RP_tmr_real[0]:.6f},{RP_tmr_real[1]:.6f},{RP_tmr_real[2]:.6f}\n"
            )
