...
if __name__ == "__main__":
    ...
    opt.data_dir = '/mnt/user/lh3/MotionStreamer-main/humanml3d_272'
    ...
    ckpt_path = pjoin(opt.checkpoints_dir, opt.dataset_name, opt.vae, 'model', opt.sae_ckpt_name)
    ...
    stage_models = molingo_models if opt.second_stage == 'molingo' else hiflow_models
    stage_prefix = 'molingo' if opt.second_stage == 'molingo' else 'hiflow'
    partial_molingo = getattr(stage_models, f'{stage_prefix}_{opt.model_size}')()
    model_kwargs = dict(
    ...
    if opt.second_stage == 'hiflow':
        model_kwargs['time_patch'] = opt.time_patch
        model_kwargs['scales'] = opt.hiflow_scales
